...
def counting_sort(nums):
    # maxm and minimum values of the nums array for finding range of queries
    maxm= 0
    minm= 2001
    # finding maximum and minimum elements of the nums array
    for i in range(nums.get_len()):
        maxm= max(maxm, nums.values[i])
        minm= min(minm, nums.values[i])
    count_len= maxm - minm +1
    # count array to store occurance count of each element of nums array
    count= Array(list([0]*count_len), 0)
    # output array to store sorted elements of nums array
    output= Array([0]*nums.get_len(), 0)
    # setting occurence count of each element of nums array
    for i in range(nums.get_len()):
        idx= nums.values[i] - minm
        val2= count.values[idx] +1
        count.set_at_idx(idx, val2)
    # changing count array to find position of each element of nums array in sorted order
    for i in range(1, count_len):
        temp= count.values[i-1]
        temp2= count.values[i]
        add= temp+temp2
        count.set_at_idx(i, add)
    # setting output array with sorted elements
    for i in range(nums.get_len()):
        idx= count.values[nums.values[i] - minm] -1
        output.set_at_idx(idx, nums.values[i])
        count.set_at_idx(nums.values[i]- minm, idx)
    # copying the sorted output array to original array
    for i in range(nums.get_len()):
        nums.set(i, output.values[i])
